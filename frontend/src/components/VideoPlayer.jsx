    ...
    <div className="fixed inset-0 z-50 bg-black/95 backdrop-blur-sm">
      <div className={`relative w-full h-full flex items-center justify-center transition-all duration-300 ${
        ...
        <div className={`relative bg-black rounded-lg overflow-hidden shadow-2xl transition-all duration-300 ${
          isFullscreen ? 'w-full h-full' : 'w-full max-w-6xl h-[70vh]'
        }`}>
          ...
          {/* Control Overlay */}
          <div className="absolute top-0 left-0 right-0 bg-gradient-to-b from-black/60 to-transparent p-4 opacity-0 hover:opacity-100 transition-opacity duration-300">
        ...
        {!isFullscreen && (
          <Button
            onClick={onClose}
            className="absolute top-4 right-4 bg-black/50 hover:bg-black/70 text-white rounded-full p-2 backdrop-blur-sm"
          >
            <X className="h-6 w-6" />
          </Button>
        )}
